...
int	ft_organize_philosophers(t_all *all, int argc, char **argv, int i)
{
	...
	while (i < all->rules.number)
	{
        ...
        if (i + 1 == all->rules.number)
            ft_philosophers(&all->philo[i], &all->philo[0], i);
        else
	    	ft_philosophers(&all->philo[i], &all->philo[i + 1], i);	
		i++;
	}
	return (0);
}
...
int	main(int argc, char **argv)
{
	...
	all.dead = &dead;
	if (ft_organize_philosophers(&all, argc - 1, argv, i))
		return (1);
	ft_logic(&all, -1);
	free(all.philo);
	return (0);
